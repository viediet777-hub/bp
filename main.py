...
# ============================================================
# VC GATEWAY PRO - AUTO PAYMENT VERIFICATION
# ============================================================
VC_GATEWAY_API_KEY = os.environ.get("VC_GATEWAY_API_KEY", "PAY3C0023FD16FC822035173195")
VC_GATEWAY_UPI_ID = os.environ.get("VC_GATEWAY_UPI_ID", "paytm.s1dw5n0@pty")  # UPI ID for QR (selling bot qr gen folder)
VC_GATEWAY_UPI_NAME = os.environ.get("VC_GATEWAY_UPI_NAME", "VC Payment Gateway")  # Payee name (selling bot qr gen folder)
VC_GATEWAY_API_URL = "https://vcgatewaypro.com/payment_api.php"
VC_GATEWAY_API_URL_FALLBACK = "https://vcapi.vcstore.site/payment_api.php"
VC_GATEWAY_CHECK_INTERVAL = int(os.environ.get("VC_GATEWAY_CHECK_INTERVAL", "15"))  # seconds


def vc_api_key():
    """Gateway API key - admin panel setting wins, else env/default (set_vc_getwayapi/savegetwayapi logic)."""
    try:
        k = (setup().get("vc_api_key") or "").strip()
    except Exception:
        k = ""
    return k or VC_GATEWAY_API_KEY


def vc_upi_id():
    """UPI ID for QR - setup override else default (selling bot qr gen folder: paytm.s1dw5n0@pty)."""
    try:
        u = (setup().get("vc_upi_id") or "").strip()
    except Exception:
        u = ""
    return u or VC_GATEWAY_UPI_ID
...
# ============================================================
def upi_amount(n):
    """Amount UPI/API ke liye integer format (selling bot qr gen/set_amount logic).
    25.0 -> '25', 25.9 -> '25' (floor), min '1'. Gateway integer amount match karta hai."""
    try:
        import math as _math
        f = float(n)
        if f != f or f == float("inf") or f == float("-inf"):
            return "1"
        return str(max(1, int(_math.floor(f))))
    except Exception:
        return str(n)


# ============================================================
# MANUAL QR PAYMENT (fallback - admin manual QR)
# ============================================================
def get_manual_qr():
    """Admin panel se set manual QR return karta hai (fallback)."""
    s = setup()
    return s.get("qr_file_id") or s.get("upi_id") or None


# ============================================================
# VC GATEWAY - AUTO QR + AUTO VERIFY (selling bot qr gen folder logic)
# ============================================================
def vc_generate_qr_url(order_id, amount):
    """QR generate karo - set_amount/qrgenerate.txt wala exact format.
    pa=paytm.s1dw5n0@pty, pn=VC Payment Gateway, tid=tr=txnid, tn=VC Payment, am=integer, cu=INR.
    QR: quickchart.io size=1000 margin=4 ecLevel=H format=png."""
    ...
    upi = ("upi://pay?pa={}"
           ...
           "&tn={}"
           "&am={}"
           "&cu=INR").format(vc_upi_id(),
                             urllib.parse.quote(VC_GATEWAY_UPI_NAME),
                             urllib.parse.quote(str(order_id)),
                             urllib.parse.quote(str(order_id)),
                             urllib.parse.quote("VC Payment"),
                             urllib.parse.quote(str(amt)))
    q = urllib.parse.quote(upi)
    # Primary: quickchart.io (folder logic). Fallbacks: qrserver + goqr (same UPI payload).
    qr_urls = [
        ("https://quickchart.io/qr"
         "?text=" + q +
         "&size=1000&margin=4&ecLevel=H&format=png"),
        ("https://api.qrserver.com/v1/create-qr-code/"
         "?size=1000x1000&data=" + q),
        ("https://goqr.me/api/?data=" + q + "&size=1000x1000"),
    ]
    return qr_urls, upi


def vc_check_payment(order_id, amount):
    """Payment check karo - check_payment (3).txt logic.
    Primary: vcgatewaypro.com, fallback: vcapi.vcstore.site (dono same params).
    Har URL pe 3 retry + backoff (SSL timeout / connection reset handle)."""
    import time as _time
    amt = upi_amount(amount)
    key = vc_api_key()
    urls = [
        "{}?api_key={}&order_id={}&amount={}".format(
            VC_GATEWAY_API_URL, urllib.parse.quote(str(key)),
            urllib.parse.quote(str(order_id)), urllib.parse.quote(str(amt))),
        "{}?api_key={}&order_id={}&amount={}".format(
            VC_GATEWAY_API_URL_FALLBACK, urllib.parse.quote(str(key)),
            urllib.parse.quote(str(order_id)), urllib.parse.quote(str(amt))),
    ]
    for url in urls:
        for attempt in range(3):
            try:
                req = urllib.request.Request(url, headers={"User-Agent": "ViedietShop/1.0"})
                with urllib.request.urlopen(req, timeout=30) as resp:
                    return json.loads(resp.read().decode("utf-8"))
            except Exception as e:
                _print_error("vc_check_payment", e)
                if attempt < 2:
...
def vc_credited_amount(res):
    """payment_result logic: amount_credited || amount."""
    try:
        return float((res or {}).get("amount_credited", (res or {}).get("amount", 0)) or 0)
    except Exception:
        return 0


def vc_status_message(res):
    """payment_result logic: pending message dikhao."""
    if not res:
        return "Gateway Error"
    return str(res.get("message") or res.get("gateway_message") or "Pending")


def vc_verify_and_deliver(order):
    """Check API -> success + credited>=total ho to auto_deliver karo.
    Returns (delivered_bool, response_dict)."""
    res = vc_check_payment(order.get("txn_id"), order.get("total"))
    if not res:
        return False, None
    if str(res.get("status", "")).lower() == "success":
        credited = vc_credited_amount(res)
        try:
            need = float(order.get("total", 0) or 0)
        except Exception:
            need = 0
        if credited >= need:
            order["credited"] = credited
            save_data()
            ok = bool(auto_deliver(order))
            return ok, res
    return False, res


def vc_payment_worker():
    """Background worker - pending orders auto-verify (I HAVE PAID / CHECK dabaye bina bhi)."""
    while True:
        time.sleep(VC_GATEWAY_CHECK_INTERVAL)
        try:
            pending_orders = [o for o in DATA["orders"] if o.get("status") == "pending" and o.get("txn_id")]
            for order in pending_orders:
                ok, _res = vc_verify_and_deliver(order)
                if ok:
                    print("[vc] auto-delivered", order.get("id"))
        except Exception as e:
            _print_error("vc_payment_worker", e)
...
def create_order(uid, username, pid, qty):
    p = DATA["products"][pid]
    DATA["seq"]["order"] += 1
    order_id = "ORD" + str(DATA["seq"]["order"]).zfill(4)
    # Gateway txn id - set_amount (2).txt wala exact format: "VC" + timestamp_ms + telegramid
    # (gateway isi format pe payment match karta hai - ORDVCG wala fail hota hai)
    try:
        txn_id = "VC" + str(int(time.time() * 1000)) + str(int(uid))
    except Exception:
        txn_id = "VC" + str(int(time.time() * 1000))
    total = p.get("price", 0) * qty
    try:
        import math as _math
        total = max(1, int(_math.floor(float(total))))
    except Exception:
        pass
    order = {
        "id": order_id,
...
def cb_myorders(cid, uid):
    ...
    for o in mine:
        ...
        if o["status"] == "pending":
            rows.append([btn("🔄 Check Payment", "check:" + o["id"], "primary", ICON_PRIMARY)])
    if not rows:
...
def cb_pay_accept(cid, uid, pid, cb_id, cb=None):
    ...
    amt = upi_amount(order["total"])
    cap = ("╭━━━━━━━━━━━━━━━━━━━━╮\n"
           "💳 <b>VC PAYMENT GATEWAY</b>\n"
           "╰━━━━━━━━━━━━━━━━━━━━╯\n\n"
           "💰 <b>Amount:</b> ₹{}\n"
           "🆔 <b>Order ID:</b> <code>{}</code>\n\n"
           "━━━━━━━━━━━━━━━━━━━━\n\n"
           "📲 <b>Scan the QR code to pay.</b>\n\n"
           "⚠️ <b>Please pay the exact amount shown above.</b>\n\n"
           "━━━━━━━━━━━━━━━━━━━━\n\n"
           "🛒 {} x{} | 🧾 #{}\n\n"
           "<b>After payment, click CHECK PAYMENT.</b>").format(
        amt, esc(order["txn_id"]),
        esc(p.get("name", "")), q, esc(order["id"]))
    rows = [
        [btn("✅ CHECK PAYMENT", "check:" + order["id"], "success", ICON_SUCCESS)],
        [btn("📦 My Orders", "myorders")],
        [btn("❌ Cancel", "home", "danger", ICON_DANGER)]
    ]
    # Auto QR generate karo (selling bot qr gen folder logic + fallback services)
    qr_urls, upi_link = vc_generate_qr_url(order["txn_id"], order["total"])
    ...
    if sent is None:
        for qr_url in qr_urls:
            ...
            if sent is not None:
                break
    if sent is None:
        send_message(cid, cap + "\n\n<b>UPI Link:</b> <code>{}</code>\n\n<i>QR load nahi hua, upar wale link pe click karke pay karein.</i>".format(esc(upi_link)), kb(rows))
    notify_admins(order)


def cb_paid(cid, uid, pid, cb_id, uname="user"):
    """User ne 'I HAVE PAID' dabaya - VC Gateway se auto verify karo."""
    ...
    # Check payment via VC Gateway (check_payment + payment_result logic)
    ok, res = vc_verify_and_deliver(o)
    if ok:
        return
    ...
    send_message(cid,
        ...
        "📌 <b>Status:</b> {}\n"
        "━━━━━━━━━━━━━━━━━━\n\n"
        "⚠️ Please complete your payment and try again.".format(
            esc(o.get("txn_id", o["id"])), esc(vc_status_message(res))),
        kb([[btn("✅ CHECK PAYMENT", "check:" + o["id"], "success", ICON_SUCCESS)],
            [btn("📦 My Orders", "myorders")], [btn("🏠 Home", "home")]]))
...
def cb_check(cid, uid, oid, cb_id):
    """Manual check payment via VC Gateway."""
    answer_cb(cb_id, "Payment verify ho raha hai... ⏳")
    o = next((x for x in DATA["orders"] if x["id"] == oid and x["userId"] == uid), None)
    if not o:
        return send_message(cid, "😕 Order not found.", kb([[btn("🏠 Home", "home")]]))
    if o["status"] != "pending":
        return send_message(cid,
...
def auto_deliver_order(cid, o):
    """Auto-deliver order when VC Gateway payment is verified."""
    return auto_deliver(o)
...
def ap_settings(cid):
    s = setup()
    # VC Gateway config - admin setting wins (set_vc_getwayapi/savegetwayapi logic)
    vc_key = vc_api_key()
    vc_upi = vc_upi_id()
    vc_name = VC_GATEWAY_UPI_NAME
    send_message(cid,
                 "🛠️ <b>Settings</b>\n"
                 "━━━━━━━━━━━━━━━\n"
                 "🏷️ Name: <b>{}</b>\n"
                 "💬 Tagline: <i>{}</i>\n"
                 "🏦 UPI (Manual): <code>{}</code>\n"
                 "📱 QR (Manual): {}\n"
                 "🆘 Support: {}\n"
                 "👋 Welcome: {}\n"
                 "📜 Terms: {}\n\n"
                 "💳 <b>VC Gateway Pro (Auto-Pay)</b>\n"
                 "━━━━━━━━━━━━━━━\n"
                 "🔑 API Key: <code>{}</code>\n"
                 "🏦 UPI ID: <code>{}</code>\n"
                 "🏷️ Payee Name: <code>{}</code>\n"
                 "⚡ Status: <b>Active</b> (auto-verify enabled)\n\n"
...
def handle_callback(upd):
    ...
    if data.startswith("as:"):
        ...
        if field == "vcapi":
            return send_message(cid, "🔑 <b>Send New Gateway API Key</b>\n\n"
                                     "━━━━━━━━━━━━━━━━━━\n"
                                     "🔹 Please send your new <b>Gateway API Key</b>.\n"
                                     "⚠️ Make sure the key is valid.\n"
                                     "━━━━━━━━━━━━━━━━━━")
...
def handle_text(upd):
    ...
    if ttype.startswith("set:"):
        ...
        if field == "brand":
            ...
            s["vc_api_key"] = text.strip()
            save_data()
            del ADMIN_DRAFT[uid]
            return send_message(cid, "✅ <b>Gateway API Key Updated Successfully</b>\n\n"
                                     "━━━━━━━━━━━━━━━━━━\n"
                                     "🔑 <b>New API Key:</b>\n"
                                     "<code>{}</code>\n"
                                     "━━━━━━━━━━━━━━━━━━\n\n"
                                     "🎉 <b>Configuration saved successfully.</b>".format(esc(text.strip())),
                                kb([[btn("Back to panel", "admin")]]))
        elif field == "welcome":
            s["welcome"] = text
        elif field == "support":
            s["support"] = text
...
def main():
    ...
    # Start VC Gateway payment verification worker
    threading.Thread(target=vc_payment_worker, daemon=True).start()
    print("Viediet Shop bot is running! (Ctrl+C to stop)")
    print("VC Gateway Pro auto-verification: ENABLED")
